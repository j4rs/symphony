defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server adds configured workspace-relative writable subpaths to the turn sandbox policy" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-writable-subpaths-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      codex_binary = Path.join(test_root, "fake-codex")
      trace_file = Path.join(test_root, "codex-writable-subpaths.trace")
      previous_trace = System.get_env("SYMP_TEST_CODEx_TRACE")
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${SYMP_TEST_CODEx_TRACE:-/tmp/codex-writable-subpaths.trace}"
      count=0
      ...
      issue = %Issue{
        id: "issue-writable-subpaths",
        identifier: "MT-1002",
        title: "Validate workspace-relative writable subpaths",
        description: "Ensure configured subpaths are added to the turn sandbox writable roots",
        state: "In Progress",
      ...
      write_workflow_file!(Workflow.workflow_file_path(),
        workspace_root: workspace_root,
        codex_command: "#{codex_binary} app-server",
        codex_workspace_writable_subpaths: [".git"]
      )

      assert {:ok, _result} = AppServer.run(workspace, "Validate writable subpaths", issue)
      ...
      assert Enum.any?(lines, fn line ->
               if String.starts_with?(line, "JSON:") do
                 ...
                 |> then(fn payload ->
                   ...
                   roots = get_in(payload, ["params", "sandboxPolicy", "writableRoots"])

                   payload["method"] == "turn/start" && is_binary(cwd) && is_list(roots) &&
                     Path.join(cwd, ".git") in roots
                 end)
               else
                 false
               end
